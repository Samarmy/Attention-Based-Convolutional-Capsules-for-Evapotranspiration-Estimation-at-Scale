...
class TrainResAttnCap_NoSSEBop():
    ...
    def __init__(self, epochs=300, batch_size=32, torch_type=torch.float32, split=1, trial=1):
        ...
        self.model_name = "ResAttnCap_NO_SSEBOP"
    ...
    def train(self):
        ...
        for epoch in range(self.epochs):
            ...
            for ind, (img_seq, true_et, veg, clim, day_sin, day_cos, x_coord, y_coord, z_coord, elev, date, lon, lat, file_name) in enumerate(self.train_loader):
                ...
                output = self.model(img_seq[:, 0:11], torch.cat((clim, veg, day_sin, day_cos, x_coord, y_coord, z_coord, elev), dim=1))
...
#             f.close()
    ...
    def final_accuracy(self, epoch):
        os.makedirs("redos2", exist_ok = True)
        with open( "redos2/" + self.model_name + "_"  + self.trial + "_" + str(self.split) + '.txt', 'w') as f:
            with torch.no_grad():
                et_correct = 0
                cfactor_correct = 0
                counter = 0
...
if __name__ == '__main__':
    ...
    trainer = TrainResAttnCap_NoSSEBop(split=int(1), trial=int(1))
    trainer.train()
